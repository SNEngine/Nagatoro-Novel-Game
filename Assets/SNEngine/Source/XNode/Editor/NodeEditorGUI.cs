...
namespace XNodeEditor
{
    ...
    public partial class NodeEditorWindow
    {
        ...
        private void DrawNodes()
        {
            ...
            // Safety: ensure culledNodes and selectionCache exist to avoid NRE when window opened not-first
            if (culledNodes == null) culledNodes = new List<XNode.Node>();
            if (selectionCache == null) selectionCache = new List<UnityEngine.Object>();
            if (_portConnectionPoints == null) _portConnectionPoints = new Dictionary<XNode.NodePort, Rect>();

            if (e.type == EventType.Layout)
            {
                selectionCache = new List<UnityEngine.Object>(Selection.objects);
                // reset culledNodes on layout
                culledNodes = new List<XNode.Node>();
            }

            // -----------------------
            // Safe search for OnValidate
            // -----------------------
            System.Reflection.MethodInfo onValidate = null;
            if (Selection.activeObject != null && Selection.activeObject is XNode.Node nodeObj)
            {
                onValidate = nodeObj.GetType().GetMethod("OnValidate",
            ...
            for (int n = 0; n < graph.nodes.Count; n++)
            {
                ...
                // -----------------------
                // Protection from null serializedObject
                // -----------------------
                if (nodeEditor == null || nodeEditor.serializedObject == null)
                    continue;
                ...
                if (e.type == EventType.Repaint)
                {
                    ...
                    foreach (var kvp in NodeEditor.portPositions)
                    {
                        ...
                        _portConnectionPoints[kvp.Key] = rect;
                    }
                }
                ...
                if (e.type != EventType.Layout)
                {
                    ...
                    if (_portConnectionPoints != null)
                    {
                        ...
                        {
                            Rect rawRect;
                            if (!_portConnectionPoints.TryGetValue(input, out rawRect)) continue;
                            Rect r = GridToWindowRectNoClipped(rawRect);
                            if (r.Contains(mousePos)) hoveredPort = input;
                        }
                        foreach (XNode.NodePort output in node.Outputs)
                        {
                            Rect rawRect;
                            if (!_portConnectionPoints.TryGetValue(output, out rawRect)) continue;
                            Rect r = GridToWindowRectNoClipped(rawRect);
            ...
            // -----------------------
            // Safe call to OnValidate
            // -----------------------
            if (onValidate != null && Selection.activeObject is XNode.Node && EditorGUI.EndChangeCheck())
            {
                try
                {
                ...
                {
                    Debug.LogWarning($"OnValidate failed: {ex.Message}");
                }
            }
        }
