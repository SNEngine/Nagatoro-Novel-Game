...
{
    public class RotateCharacterNodeWorker : SNILWorker
    {
        public override void ApplyParameters(BaseNode node, Dictionary<string, string> parameters)
        {
            // Проверяем, что это действительно RotateCharacterNode или его наследник
            if (!(node is RotateCharacterNode rotateNode))
            {
                return;
            }
