...
{
    public class DissolveCharacterNodeWorker : SNILWorker
    {
        public override void ApplyParameters(BaseNode node, Dictionary<string, string> parameters)
        {
            // Проверяем, что это действительно DissolveCharacterNode или его наследник
            if (!(node is DissolveCharacterNode dissolveNode))
            {
                return;
            }
