...
{
    public class SetColorCharacterNodeWorker : SNILWorker
    {
        public override void ApplyParameters(BaseNode node, Dictionary<string, string> parameters)
        {
            // Проверяем, что это действительно SetColorCharacterNode или его наследник
            if (!(node is SetColorCharacterNode setColorNode))
            {
                return;
            }
