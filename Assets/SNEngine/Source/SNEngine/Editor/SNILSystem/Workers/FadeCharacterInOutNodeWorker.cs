...
{
    public class FadeCharacterInOutNodeWorker : SNILWorker
    {
        public override void ApplyParameters(BaseNode node, Dictionary<string, string> parameters)
        {
            // Проверяем, что это действительно FadeCharacterInOutNode или его наследник
            if (!(node is FadeCharacterInOutNode fadeNode))
            {
                return;
            }
