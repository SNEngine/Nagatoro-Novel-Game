...
{
    public class ScaleCharacterNodeWorker : SNILWorker
    {
        public override void ApplyParameters(BaseNode node, Dictionary<string, string> parameters)
        {
            // Проверяем, что это действительно ScaleCharacterNode или его наследник
            if (!(node is ScaleCharacterNode scaleNode))
            {
                return;
            }
